...
Dieser Abschnitt setzt die Anforderungen F1 (Spielfeld) und F2 (Einheiten) um (vgl. Tabelle~\ref{tab:funktionale-anforderungen}): ein 10$\times$10-Raster mit fünf Geländearten, fraktionsspezifischen Effekten und Spawn-Zonen für zwei Fraktionen.\footnote{Der \texttt{GameBuilder} unterstützt architektonisch 2--4 Fraktionen; die aktuelle Spieloberfläche nutzt zwei.}

Die Erzeugung des Spielfelds erfolgt über den \texttt{GameBuilder} (Builder Pattern).
...
Listing~\ref{lst:builder-build} zeigt die Fluent-API des \texttt{GameBuilder} sowie die zentrale \texttt{build()}-Methode.
Die Methode \texttt{withFactions()} validiert die Fraktionsanzahl\footnote{Intern erlaubt: 2--4 Fraktionen (\texttt{MIN\_FACTIONS}=2, \texttt{MAX\_FACTIONS}=4); im Spiel werden aktuell zwei verwendet.} und gibt \texttt{this} zurück, um weitere Konfigurationsschritte zu verketten.
In \texttt{build()} wird zunächst geprüft, ob Fraktionen vorhanden sind und jede Fraktion mindestens eine Einheit besitzt; anschließend wird das \texttt{Battlefield} mit der Geländeverteilung initialisiert und die Einheiten werden in vier 3$\times$3-Spawn-Regionen an den Ecken des Spielfelds platziert (vgl. \texttt{GameBuilder.java Z.\,164ff}).
...
Listing~\ref{lst:composite-hierarchy} stellt die drei Ebenen der Composite-Hierarchie gegenüber.
\texttt{Cell} gibt als Leaf stets eine Einelementliste zurück, \texttt{Region} delegiert an ihre enthaltenen Zellen und \texttt{Battlefield} flacht alle Zeilen zu einer Gesamtliste ab.
Die Methode \texttt{applyEffect(Consumer<Cell>)} im Interface ermöglicht es, denselben Effekt -- beispielsweise das Umwandeln von Wald in Lava bei einem Waldbrand-Event -- auf eine einzelne Zelle, eine 3$\times$3-Region oder das gesamte Spielfeld anzuwenden, ohne die Logik duplizieren zu müssen (vgl. \texttt{BattlefieldComponent.java Z.\,53ff}).
...
Listing~\ref{lst:visitor-lava} zeigt das \texttt{TerrainVisitor}-Interface mit seinen vier fraktionsspezifischen \texttt{visit()}-Methoden sowie den \texttt{LavaTerrainVisitor} als konkreten Visitor.
Feuer-Einheiten erhalten auf Lava einen Angriffsbonus von~+2, Wasser-Einheiten erleiden $-5$~LP/Runde.
Für Erde und Luft greift die Standardimplementierung aus \texttt{AbstractTerrainVisitor}, die \texttt{TerrainEffectResult.NEUTRAL} zurückgibt.
Der Double Dispatch wird in der \texttt{Unit}-Klasse durch einen \texttt{switch}-Ausdruck über die Fraktion realisiert: \texttt{accept(TerrainVisitor)} ruft je nach Fraktion die passende \texttt{visit()}-Methode auf (vgl. \texttt{Unit.java Z.\,168ff}).
Auf diese Weise werden alle 20~Gelände-Fraktions-Kombinationen typsicher und ohne verschachtelte Fallunterscheidungen abgebildet.
Die \texttt{TerrainVisitorFactory} hält die fünf zustandslosen Visitor-Instanzen in einer \texttt{EnumMap} vor und liefert sie in $O(1)$ aus (vgl. \texttt{TerrainVisitorFactory.java Z.\,36ff}).
...
Dieser Abschnitt setzt Anforderung F4 (Aktionen: Bewegen und Angreifen) um (vgl. Tabelle~\ref{tab:funktionale-anforderungen}): geländeabhängige Bewegungskosten sowie Nah- und Fernkampf mit Sichtlinienprüfung.

Die Bewegungs- und Angriffslogik wird über das Strategy Pattern in zwei getrennten Hierarchien gekapselt.
...
Im Gegensatz dazu berücksichtigt die \texttt{GroundMovementStrategy} fraktionsspezifische Geländemodifikatoren.
Sie erhält im Konstruktor die Fraktion der Einheit und leitet in \texttt{calculateMovementCost()} per \texttt{switch}-Ausdruck an die passende Modifikator-Methode weiter (vgl. \texttt{GroundMovementStrategy.java Z.\,44ff}).
...
Listing~\ref{lst:strategy-ground} zeigt den Kontrast zur \texttt{FlyingMovementStrategy}: Während fliegende Einheiten pauschal Kosten~1 zahlen, variieren die Kosten für Bodeneinheiten je nach Fraktion und Gelände erheblich -- Feuer-Einheiten bewegen sich auf Lava für Kosten~1 statt~2, Wasser-Einheiten auf Eis für Kosten~1 statt~3, dafür auf Lava für Kosten~3 statt~2.
...
Dieser Abschnitt setzt Anforderung F4 (Aktionen: Undo/Redo) um (vgl. Tabelle~\ref{tab:funktionale-anforderungen}): vollständige Rücknahme aller Aktionen innerhalb einer Runde.

Das Undo/Redo-System basiert auf dem Command Pattern.
...
Listing~\ref{lst:attack-command} zeigt den \texttt{AttackCommand} im Detail.
In \texttt{execute()} wird zunächst der aktuelle Gesundheitswert des Ziels in \texttt{targetPreviousHealth} gesichert.
Anschließend berechnet der \texttt{DamageCalculator} über seine Handler-Kette (Chain of Responsibility) den Gesamtschaden, der auf das Ziel angewendet wird.
Der Aktionszähler des Angreifers wird inkrementiert und die Observer über ein \texttt{UnitAttackedEvent} benachrichtigt.
Bei \texttt{undo()} wird die Differenz zwischen gespeichertem und aktuellem Gesundheitswert per \texttt{heal()} wiederhergestellt und der Aktionszähler dekrementiert, sodass die Aktion erneut zur Verfügung steht.
