...
Die Implementierung der Einheitengenerierung als Factory Method Pattern deckt somit die Anforderung F2 (vgl. Tabelle~\ref{tab:funktionale-anforderungen}) ab.
...
Über eine Fluent-API werden zunächst Fraktionen registriert, optional eine benutzerdefinierte Geländeverteilung gesetzt und
eine beliebige Anzahl von Einheiten pro Fraktion hinzugefügt.
Die \texttt{build()}-Methode validiert die Konfiguration, erzeugt das \texttt{Battlefield} und platziert anschließend die
Einheiten in den Ecken-Spawn-Zonen.
...
Die Validierung erfolgt bewusst erst im abschließenden build()-Aufruf und nicht bei jedem einzelnen Methodenaufruf der Fluent-API.
Dadurch kann die Konfiguration in beliebiger Reihenfolge zusammengestellt werden, solange am Ende alle Voraussetzungen erfüllt
sind. Ein frühzeitiges Validieren hätte den Nachteil, dass der Aufrufer eine bestimmte Reihenfolge einhalten müsste, etwa
zuerst Fraktionen registrieren, bevor Einheiten hinzugefügt werden dürfen. Die Spawn-Logik platziert die Einheiten in den
Ecken des Spielfelds, um beiden Fraktionen maximalen Startabstand zu gewähren (vgl. \texttt{GameBuilder.java} Z.,164ff).

Das Spielfeld ist als Composite-Hierarchie aufgebaut: Das Interface \texttt{BattlefieldComponent} wird von der
\texttt{Cell} (Leaf), der \texttt{Region} (Composite) und dem \texttt{Battlefield} (Root-Composite) implementiert.
...
Entscheidend an dieser Implementierung ist, dass \texttt{Cell} sich selbst als einelementige Liste zurückgibt. Dadurch kann die
Methode \texttt{applyEffect()} auf allen drei Ebenen identisch per \texttt{forEach} iterieren, ohne dass \texttt{instanceof}-Prüfungen
oder Fallunterscheidungen zwischen Einzelzellen und Gruppen notwendig sind. Alternativ hätte man \texttt{Cell} und \texttt{Region}
mit getrennten Schnittstellen modellieren können, was jedoch dazu geführt hätte, dass Clients wie die \texttt{EventPhase}
für jede Stufe eigene Methoden benötigt hätten. Zusätzlich bietet \texttt{Battlefield} die Methode \texttt{getRegion(x1, y1, x2, y2)},
um beliebige Teilbereiche des Spielfelds zu erreichen. Die Events der \texttt{EventPhase} nutzen diese Composite-Struktur
und wenden ihre Effekte per \texttt{applyEffect()} auf Zelle, Region oder das gesamte Spielfeld an.

Die Geländeeffekte auf die spezifischen Einheiten werden über das Visitor Pattern abgebildet.
Aus 5~Geländetypen und 4~Fraktionen ergeben sich 20~Kombinationen, die ohne Visitor als fehleranfällige \texttt{if-else}-Anhäufungen
...
Der \texttt{AbstractTerrainVisitor} liefert für alle Fraktionen standardmäßig ein neutrales Ergebnis zurück. Konkrete Visitor
wie der \texttt{LavaTerrainVisitor} müssen daher nur die Methoden überschreiben, bei denen tatsächlich ein Effekt eintritt.
Im Fall von Lava sind das \texttt{visitFireUnit()} mit +2 Angriff und \texttt{visitWaterUnit()} mit −5-5 −5 Lebenspunkten
pro Runde. Die übrigen 18 der insgesamt 20 Gelände-Fraktions-Kombinationen benötigen dadurch keinen eigenen Code, was den
Implementierungsaufwand beim Hinzufügen eines neuen Geländetyps auf die tatsächlich betroffenen Fraktionen beschränkt. Das
Ergebnis jedes Visitor-Aufrufs ist ein unveränderliches \texttt{TerrainEffectResult}-Record mit Angriffs- und Verteidigungsbonus,
einem LP-pro-Runde-Effekt sowie einer optionalen Geländetransformation (z.\,B. Eis \rightarrow Wüste bei Feuer-Einheiten).
Der \texttt{Unit.accept(TerrainVisitor)} realisiert den Double Dispatch über einen \texttt{switch}
über die Fraktionen (vgl. \texttt{Unit.java} Z.\,168ff). Die \texttt{TerrainVisitorFactory} hält die fünf Visitor-Instanzen
in einer \texttt{EnumMap} und gibt diese zurück (vgl. \texttt{TerrainVisitorFactory.java} Z.\,36ff).
...
\texttt{GroundMovementStrategy} berücksichtigt fraktionsspezifische Modifikatoren (z.\,B. Feuer auf Lava: Kosten~1 statt~2, Wasser auf Lava: Kosten~3),
während \texttt{FlyingMovementStrategy} alle Geländekosten pauschal auf~1 setzt.
...
Der Phönix stellt hier einen interessanten Sonderfall dar, da er als Feuer-Einheit mit \texttt{FlyingMovementStrategy}
ausgestattet ist. Dadurch profitiert er nicht von den reduzierten Lava-Kosten seiner Fraktion, weil fliegende Einheiten
pauschal 1 Bewegungspunkt pro Feld zahlen. Das ist eine bewusste Designentscheidung: Der Vorteil des Fliegens, nämlich die
Unabhängigkeit von allen Geländekosten, wiegt den Verlust des fraktionsspezifischen Lava-Bonus auf. Hätte man beides kombiniert,
wäre der Phönix auf Lava-Feldern gegenüber anderen fliegenden Einheiten im Vorteil gewesen, ohne dass der Geländetyp für
Flieger eigentlich relevant sein sollte.

Des Weiteren zeigt das Listing~\ref{lst:strategy-ground} die \texttt{GroundMovementStrategy} mit fraktionsspezifischen Geländemodifikatoren (vgl. \texttt{GroundMovementStrategy.java Z.\,44ff}).
...
Die fraktionsspezifischen Modifikatoren folgen dem Prinzip, dass jede Fraktion auf ihrem zugehörigen Gelände vergünstigte
Kosten erhält, auf dem entgegengesetzten Element jedoch einen Aufschlag zahlt. Wasser auf Eis kostet beispielsweise 1 statt 3,
auf Lava dagegen 3 statt 2. Diese Asymmetrie soll verhindern, dass Spieler ihre Einheiten ausschließlich über das eigene
Elementargelände bewegen, und stattdessen taktische Entscheidungen über die Routenwahl erzwingen. Für die Luft-Fraktion
entfällt diese Mechanik vollständig, da alle ihre Einheiten die \texttt{FlyingMovementStrategy} verwenden und somit
geländeunabhängig agieren.

Die \texttt{AttackStrategy} definiert die Methoden \texttt{canAttack()}, \texttt{calculateBaseDamage()} und \texttt{getAttackRange()}.
\texttt{MeleeAttackStrategy} prüft die Distanz auf die Reichweite der Einheit (typischerweise~1).
Die \texttt{RangedAttackStrategy} validiert zusätzlich die Sichtlinie. Im Wald-Gelände auf dem Pfad zwischen Angreifer
und Ziel blockiert der Angriff, sobald die Einheit nicht über die Eigenschaft \texttt{ignoresForestDefense} verfügt (vgl. \texttt{RangedAttackStrategy.java Z.\,52ff}).
Alle 12 konkreten Einheiten setzen ihre Strategien explizit im Konstruktor: 4~Einheiten erhalten \texttt{FlyingMovementStrategy}
(Phoenix, WindDancer, StormCaller, SkyGuardian), 5~Einheiten \texttt{RangedAttackStrategy}.
Die Getter in \texttt{Unit} (Z.\,142ff) enthalten \texttt{GroundMovementStrategy} bzw. \texttt{MeleeAttackStrategy} als Fallback.

\subsubsection{Schadensberechnung (Christian Stiens)}
\label{subsec:schadensberechnung}

Die Schadensberechnung wird über das Chain of Responsibility Pattern abgebildet und gehört zur Aktion \texttt{Angreifen} aus Anforderung F4 (vgl. Tabelle~\ref{tab:funktionale-anforderungen}).
...
public class DamageCalculator {
    ...
    public DamageCalculator() {
        ...
        handlerChain
            ...
            .setNext(new TerrainEffectHandler())     // Visitor
            .setNext(new SynergyBonusHandler())      // Decorator
...
Der erste Handler, \texttt{BaseDamageHandler}, liest den Grundschaden über das Strategy Pattern aus
(\texttt{attacker.getAttackStrategy()}\newline\texttt{.calculateBaseDamage()}) und schreibt ihn in den Kontext (vgl. \texttt{BaseDamageHandler.java Z.\,14ff}).
...
Im Verlauf des Spiels wird mit rundenweise steigender Wahrscheinlichkeit (max. 60\,\%) zu Beginn jedes Spielerzugs einer von sechs Effekten auf eine zufällig ausgewählte, lebende Einheit der aktiven Fraktion angewendet (Decorator Pattern, vgl. \texttt{BuffDebuffManager.java Z.\,41ff}, Kapitel 2.3.3).
Diese Mechanik wird Anforderung F3 (vgl. Tabelle~\ref{tab:funktionale-anforderungen}) zugerechnet.
...
// Konkrete Decorator-Implementierung
...
Die \texttt{CommandHistory} verwendet zwei getrennte Stacks statt einer einzelnen Liste mit Index, da die Stack-Operationen
\texttt{push} und \texttt{pop} die Undo/Redo-Logik direkt abbilden und keine zusätzliche Indexverwaltung erfordern. Ein bewusster
Trade-off zeigt sich in Zeile~7: Sobald nach einem Undo ein neuer Befehl ausgeführt wird, wird der gesamte Redo-Stack geleert.
Das bedeutet, dass der Spieler eine einmal verworfene Aktionsfolge nicht wiederherstellen kann, wenn er zwischenzeitlich eine
andere Aktion ausführt. Ein Branching-Modell, bei dem mehrere Aktionsverläufe parallel vorgehalten werden, wäre für ein
Rundenstrategie-Spiel unverhältnismäßig komplex gewesen. Die Einschränkung auf Per-Turn-Rollback durch \texttt{clear()} bei
Rundenwechsel begrenzt zudem den Speicherverbrauch und verhindert, dass Spieler Züge aus vergangenen Runden rückgängig machen,
was die Spielbalance beeinträchtigen würde.
...
Der \texttt{AttackCommand} ist zugleich der Auslöser für die Siegbedingungsprüfung (Anforderung F5): Fällt eine Einheit unter 0~LP,
ruft er \texttt{game.handleUnitDeath()} auf. Diese Methode entfernt die Einheit, benachrichtigt die Observer und prüft
anschließend über die Methode \texttt{checkVictoryCondition()}, ob nur noch eine Fraktion lebende Einheiten besitzt. Ist
dies der Fall, wechselt das Spiel in die \texttt{GameOverPhase} (State Pattern). Die gleiche Prüfung erfolgt auch bei
jedem Zugendewechsel über \texttt{endTurn()}.

Der \texttt{RoundManager} zählt die Spielrunden mit: Er wird bei Spielstart auf~1 gesetzt und inkrementiert, sobald der Zug
zur ersten Fraktion zurückkehrt. Die \texttt{CommandHistory} nutzt den Rundenwechsel als Grenze und leert bei jedem neuen Zug beide Stacks.
