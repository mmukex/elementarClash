...
Die Implementierung der Patterns wurde zwischen beiden Autoren aufgeteilt.
...
2 & Builder & Erz. & Schrittweise Spielfeld-Erstellung & 100 Zellen, Geländeverteilung, Spawn-Positionen \\
...
6 & State & Verh. & Spielphasen und Einheitenzustände & Zustandsabhängiges Verhalten \\
7 & Observer & Verh. & Event-System, UI-Entkopplung & Lose Kopplung zwischen Spiellogik und Darstellung \\
8 & Command & Verh. & MoveCommand, AttackCommand + Undo/Redo & Aktionen mit Rollback-Fähigkeit \\
...
Das State Pattern und Observer Pattern beschreiben den Ablauf: Spielphasen steuern den Kontrollfluss, Observer entkoppeln die UI von der Spiellogik.
...
Ohne ein Erzeugungsmuster müsste der Aufrufer jeden konkreten Konstruktor kennen und die passenden Stat-Werte selbst übergeben.
Dies wäre eine Kopplung, die bei einer neuen Fraktion Änderungen an mehreren Stellen im Code erfordern würde.
...
Die konkreten Einheitenklassen sind package-private, sodass sie ausschließlich über ihre Fabrik instanziiert werden können. (vgl. \cite{entwurfsmuster})
...
\begin{figure}[H]
	\centering
	\includegraphics[width=0.9\textwidth]{images/Builder_Pattern___ElementarClash}
...
\textbf{(5)} \texttt{DefenseCalculationHandler} ist der terminale Handler und berechnet den Endschaden nach $\text{finalDamage} = \max(1,\ \text{totalAttack} - \text{totalDefense})$, damit ein Minimalschaden von~1 garantiert ist.
