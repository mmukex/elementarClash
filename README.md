...
| **Rundenbasierte Logik**    | Pro Einheit pro Runde: **1× Bewegen** (geländeabhängig) + **1× Angreifen oder Fähigkeit einsetzen**.        |
...
|-----------|--------------------------------|--------------------|--------------------------------------------------------------------------------------------|---------------------------------------------------|  
| **Lava**  | Normal: 2, Feuer: 1, Wasser: 3 | 0                  | **Feuer:** +2 Angriff / **Wasser:** -5 LP/Runde                                            | Wird durch Waldbrand-Ereignis erschaffen          |  
| **Eis**   | Normal: 3, Wasser: 1, Feuer: 2 | +1 Verteidigung    | **Wasser:** +2 Verteidigung, Heilt 5 LP/Runde / **Feuer:** Schmilzt zu Wüste nach Bewegung | Wird durch Geysir-Ereignis erschaffen             |  
...
| 2  | **Builder**                 | Erzeugung     | Schrittweise Erstellung des Spielfelds (10×10 Raster mit Geländeverteilung, Einheiten-Platzierung, Validierung)            | Das Battlefield ist komplex: 100 Zellen, zufällige Geländeverteilung (5 Typen mit Prozent-Vorgaben), faire Startpositionen für 2-4 Fraktionen, verschiedene Szenarien (Standard, Eis-Welt, Vulkan-Arena). Builder ermöglicht flexible Konfiguration und wiederverwendbare Setup-Logik.                                                                                                       | @mmukex  |  
| 3  | **Composite**               | Struktur      | Raster-Hierarchie (Battlefield → Zeilen → Zellen) und Einheitengruppen (Armee vs. einzelne Einheit)                        | Das 10×10-Raster enthält 100 Zellen. Composite ermöglicht Operationen sowohl auf einzelnen Zellen als auch auf ganzen Regionen (z.B. "applyForestFire()"). Einheitengruppen ermöglichen Synergien wie "+1 Angriff für benachbarte Feuer-Einheiten".                                                                                                                                          | @mmukex  |  
| 4  | **Decorator**               | Struktur      | Stapeln temporärer Buffs/Debuffs auf Einheiten                                                                             | Einheiten erhalten dynamische, stapelbare Boni: Synergien (benachbarte Feuer-Einheiten: +1 Angriff), Fähigkeits-Buffs (Feuersturm: +3 Angriff für 2 Runden), Status-Effekte (Verlangsamung, Betäubung). Decorator ermöglicht beliebiges Stapeln mehrerer Effekte ohne Änderung der Unit-Klasse.                                                                                            | @crstmkt |  
...
| 6  | **State**                   | Verhalten     | Einheiten-Zustände (Idle, Moving, Attacking, Stunned, Dead) und Spielphasen (Setup, PlayerTurn, EventPhase, GameOver)      | Einheiten können nur bestimmte Aktionen in bestimmten Zuständen ausführen (kein Angriff wenn betäubt). Das Spiel hat klare Phasen (Einheiten bewegen → dynamische Ereignisse → nächster Spieler). State Pattern verwaltet Übergänge sauber.                                                                                                                                                    | @crstmkt |  
...
| 8  | **Command**                 | Verhalten     | Spieler-Aktionssystem (MoveCommand, AttackCommand, UseAbilityCommand) mit eingebetteter Validierungs- und Ausführungslogik | Kapselt alle Spielzüge als Objekte. Pro Einheit pro Runde: 1 Move + 1 Attack/Ability Command. Jedes Command enthält eigene Validierung (Reichweite, Cooldowns, Bewegungs-/Angriffs-Status), Ausführungslogik und Rollback-Fähigkeit. Ermöglicht Undo/Redo mit per-turn Command History. Commands delegieren Validierung an Strategy Pattern (MovementStrategy, AttackStrategy). | @mmukex  |  
| 9  | **Chain of Responsibility** | Verhalten     | Schadensberechnung-Pipeline (Basisschaden → Elementar-Modifikator → Synergien → Verteidigung)                              | Schaden wird in sequenziellen Schritten berechnet: Einheiten-Angriff, elementare Vorteile (Feuer vs Erde: +25%, vs Wasser: -25%), Synergien (benachbarte Einheiten), Ziel-Verteidigung. Jeder Handler fügt seine Modifikation hinzu und gibt an den nächsten weiter. Chain ermöglicht flexible Erweiterung um neue Schadens-Modifikatoren ohne bestehenden Code zu ändern.           | @crstmkt |  
...
Bewegung: ✓ | Angriff: ✓                  Gesamt-LP: 270/270
═══════════════════════════════════════════════════════════════
